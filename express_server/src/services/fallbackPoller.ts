...
export const startFallbackPoller = () => {
    ...
    setInterval(async () => {
        try {
            const unprocessedTickets = await Ticket.find({ aiProcessed: false, status: 'Open' });
            ...
            const recentOpenTickets = await Ticket.find({ status: 'Open', aiProcessed: true }).limit(50);
            ...
            for (const ticket of unprocessedTickets) {
                try {
                    ...
                    const transRes = await axios.post(`${DL_SERVER_URL}/translate`, { text: ticket.description });
